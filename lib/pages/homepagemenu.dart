...
import 'package:goshare/pages/screen.dart';
...
import '../Utilidades/endpoinst.dart';
import '../Utilidades/settings.dart';
...
//cria cada item do menu
  ...
  Widget RetornaCardEmpresa(Empresa empresa) {
    return Container(
                  ...
                  child: Card(
                    ...
                    child: TextButton(
                      child: Container(
                        ...
                        child: Center(
                            child: Text(
                                "Campos de endereço incompletos. Atualize seu endereço no menu editar perfil para continuar.\n\nOK",
      ...
      //             SizedBox(height: 5),
      //             Center(child: Text(
      //               (((empresa.funcionamento??"").split('##').length >=2) ? 
      //               (empresa.funcionamento??"").split('##')[1] : 
      //               ""), style: GoogleFonts.mcLaren(fontSize: 11,fontWeight: FontWeight.bold,color: Colors.green[600]))
      //             ),
      //             SizedBox(height: 5),
      //             Row(children: <Widget>[
      //               Container(
    ...
    if (Utilidades.start % 2 == 0) {
      try {
        ...
        for (int i = 0; i < response_.data['data'].length; i++) {
          ...
          //if (response_.data["data"][i]["companyName"].contains("##") || response_.data["data"][i]["companyName"].toString() == "CompreAqui") {
  ...
  List<Widget> RetornaWidgetsEmpresas_(BuildContext context) {
    ...
    try {
      ...
      listaWidget.add(listaFechadas.length >0 ? 
        Center(child: Text("Inativos:", style: GoogleFonts.mcLaren(fontSize: 16,fontWeight: FontWeight.bold)))
        : Text(""));
          ...
          ListTile(
            ...
            title: Container(child: 
                Row(children: <Widget> [
                    ...
                    SizedBox(width: 5,),
                    Container(width: 10, height: 10, child: (Utilidades.novaMensagem ? Image.asset('assets/ponto_escuro.png') : Text("")))
                  ])
            )
          ),
          ListTile(
            ...
            title: Container(child: 
              Row(children: <Widget> [
                    ...
                    SizedBox(width: 5,),
                    Container(width: 10, height: 10, child: (((Utilidades.usuarioLogado.cidade == null || Utilidades.usuarioLogado.cidade == "") ? true: false) ? Image.asset('assets/ponto_escuro.png') : Text("")))
                  ]))
          ),
          ListTile(
            leading: Icon(Icons.build),
            onTap: () => IrParaEdicaoSenha(),
            title: Container(child: Text("ALTERAR SENHA",style:  GoogleFonts.mcLaren(fontSize: 12,fontWeight: FontWeight.bold) ) )
          ),
          ListTile(
            leading: Icon(Icons.bookmark),
            onTap: () => IrParaTermosUso(),
            title: Container(
              child:
                Text("TERMOS DE USO",style:  GoogleFonts.mcLaren(fontSize: 12,fontWeight: FontWeight.bold),)
            )
          ),
          // ListTile(
          //   leading: Icon(Icons.map),
          //   onTap: () => IrParaLocalizacao(),
          //   title: Container(
          //     child: 
          //       Text("LOCALIZAÇÃO",style:  GoogleFonts.mcLaren(fontSize: 12,fontWeight: FontWeight.bold),),
          //   )
          // ),
          ListTile(
            leading: Icon(Icons.star),
            onTap: () => IrParaAvaliacao(),
            title: Container(child: 
                Text("AVALIAÇÃO",style:  GoogleFonts.mcLaren(fontSize: 12,fontWeight: FontWeight.bold),),
            )
          ),
          ListTile(
            ...
            onTap: () => IrParaSobre(),
            title: Container(child:
              Text("MAIS",style:  GoogleFonts.mcLaren(fontSize: 12,fontWeight: FontWeight.bold),),
            )
          ),
          Divider(),
          ListTile(
            ...
            onTap: () => Sair(),
            title: Container(child: 
              Text("SAIR",style:  GoogleFonts.mcLaren(fontSize: 12,fontWeight: FontWeight.bold),),
            )
          ),                ]
          )
        ),
