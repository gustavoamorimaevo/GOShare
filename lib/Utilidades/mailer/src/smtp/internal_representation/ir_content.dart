...
abstract class _IRContentPart extends _IRContent {
  ...
  Stream<List<int>> out(_IRMetaInformation irMetaInformation) async* {
    ...
    if (!_active) {
      ...
      var content_ = _content?.first;
      //nesse caso nao haja elemento uma exception será disparada;
      yield* content_?.out(irMetaInformation) ?? [].first;
      return;
    }
