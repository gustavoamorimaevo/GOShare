...
import 'package:goshare/Utilidades/mailer/src/smtp/exceptions.dart';
import 'package:goshare/Utilidades/mailer/smtp_server.dart';
...
class Connection {
  ...
  DateTime get connectionOpenStart => _connectionOpenStart?? new DateTime(2023,1,1,00,00,00);
  ...
  Future sendStream(Stream<List<int>> s) => _socket?.addStream(s) ?? Future.value(false);
  ...
  Future<ServerResponse> send(String command,
    ...
    if (!waitForResponse) {
      ...
      return Future(() => new ServerResponse("404", ["-"]));
    }
  ...
  Future<void> upgradeConnection() async {
    ...
    //comentado para tratamento do flutter v2
  ...
  void _setSocketIn() {
    ...
    //comentado tratamento flutter v2
